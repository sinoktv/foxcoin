...
// Foxcoin: Normally minimum difficulty blocks can only occur in between
...
unsigned int CalculateFoxcoinNextWorkRequired(const CBlockIndex* pindexLast, int64_t nFirstBlockTime, const Consensus::Params& params)
{
...
CAmount GetFoxcoinBlockSubsidy(int nHeight, const Consensus::Params& consensusParams, uint256 prevHash)
{
...
int64_t GetFoxcoinDustFee(const std::vector<CTxOut> &vout, CFeeRate &baseFeeRate) {
    int64_t nFee = 0;
