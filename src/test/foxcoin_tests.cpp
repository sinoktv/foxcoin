// Copyright (c) 2015 The Foxcoin Core developers
...
BOOST_FIXTURE_TEST_SUITE(foxcoin_tests, TestingSetup)
...
BOOST_AUTO_TEST_CASE(subsidy_limit_test)
{
    ...
    for (nHeight = 0; nHeight <= 100000; nHeight++) {
        ...
        CAmount nSubsidy = GetFoxcoinBlockSubsidy(nHeight, params, prevHash);
    ...
    //test sum +- ~10billion
    arith_uint256 upperlimit = arith_uint256("95e14ec776380000"); //108 billion FOX
    BOOST_CHECK(nSum <= upperlimit);
    
    arith_uint256 lowerlimit = arith_uint256("7a1fe16027700000"); //88 billion FOX
    BOOST_CHECK(nSum >= lowerlimit);
    
    // Test reward at 600k+ is constant
    const Consensus::Params& params = mainParams.GetConsensus(600000);
    CAmount nConstantSubsidy = GetFoxcoinBlockSubsidy(600000, params, prevHash);
    BOOST_CHECK_EQUAL(nConstantSubsidy, 10000 * COIN);

    nConstantSubsidy = GetFoxcoinBlockSubsidy(700000, params, prevHash);
    BOOST_CHECK_EQUAL(nConstantSubsidy, 10000 * COIN);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_difficulty_limit)
{
    ...
    BOOST_CHECK_EQUAL(CalculateFoxcoinNextWorkRequired(&pindexLast, nLastRetargetTime, params), 0x1e00ffff);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_pre_digishield)
{
    ...
    BOOST_CHECK_EQUAL(CalculateFoxcoinNextWorkRequired(&pindexLast, nLastRetargetTime, params), 0x1c15ea59);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_digishield)
{
    ...
    BOOST_CHECK_EQUAL(CalculateFoxcoinNextWorkRequired(&pindexLast, nLastRetargetTime, params), 0x1b671062);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_digishield_modulated_upper)
{
    ...
    BOOST_CHECK_EQUAL(CalculateFoxcoinNextWorkRequired(&pindexLast, nLastRetargetTime, params), 0x1b4e56b3);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_digishield_modulated_lower)
{
    ...
    BOOST_CHECK_EQUAL(CalculateFoxcoinNextWorkRequired(&pindexLast, nLastRetargetTime, params), 0x1b335358);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_digishield_rounding)
{
    ...
    BOOST_CHECK_EQUAL(CalculateFoxcoinNextWorkRequired(&pindexLast, nLastRetargetTime, params), 0x1b6558a4);
}
