...
*Setup instructions for a gitian build of Foxcoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Foxcoin
...
- [Building Foxcoin](#building-foxcoin)
...
Clone the git repositories for foxcoin and gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/foxcoin/foxcoin
```
...
Follow the instructions in [doc/release-process.md](release-process.md) in the foxcoin repository
...
To build Foxcoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the foxcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/foxcoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/foxcoin/foxcoin
...
For example:
```bash
URL=https://github.com/laanwj/foxcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit foxcoin=${COMMIT} --url foxcoin=${URL} ../foxcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit foxcoin=${COMMIT} --url foxcoin=${URL} ../foxcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit foxcoin=${COMMIT} --url foxcoin=${URL} ../foxcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/foxcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/foxcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/foxcoin-osx-build.assert
```
...
[foxcoin/gitian.sigs](https://github.com/foxcoin/gitian.sigs/) repository, or if that's not possible create a pull request. 
