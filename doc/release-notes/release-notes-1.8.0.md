...
Foxcoin Core 1.8 introduces AuxPoW from block 371,337. AuxPoW is a technology
which enables miners to submit work done while mining other coins, as work
on the Foxcoin block chain. Foxcoin Core 1.8 also enables payment protocol
support for Foxcoin (note that Foxcoin's implementation of payment protocol is
...
as soon as possible afterwards in order to be able to continue to use Foxcoin.
Users remaining on the previous client **will not** be able to receive or spend
Foxcoin (with the exception of with other nodes who have not upgraded).
...
use "foxcoind" you must run it the first time with the "-reindex" command line
option.
...
AuxPoW benefits Foxcoin in two ways; firstly, the effective hashrate of the coin is
...
multipools from switching between Foxcoin and other Scrypt coins.
...
In comparison to the AuxPoW implementation in United Scrypt Coin, Foxcoin uses a
different coinbase transaction input script format which includes block height in
compliance with BIP0034.

## Payment Protocol

Foxcoin Core 1.8 adds payment protocol support, which is used to streamline payments
being made to merchants.

This protocol is defined in DIP0070-DIP0072, based on the BIP standards with the same
assigned numbers. DIP standards can be found at https://github.com/foxcoin/dips/ .
The key differencs between the Bitcoin and Foxcoin payment protocols are that
the Foxcoin payment request uses a "genesis" field containing the hash of the network's
...
(typically 1 FOX, or around $0.00012 per transaction).
...
of Foxcoin compared to Bitcoin. This should lead to less false positives.

A problem in parsing mangled foxcoin: URIs under Windows has been resolved, and as
part of this work network detection for foxcoin: URIs is now more robust.
