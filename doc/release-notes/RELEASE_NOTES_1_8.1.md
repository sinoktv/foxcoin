...
Foxcoin Core 1.8.1 is primarily a bugfix release, bringing Foxcoin Core in
line with Bitcoin 0.9.3. Foxcoin Core 1.8.1 also adds in support for printing
...
* Disable Transifex based translation process (not used by Foxcoin Core)
* Add checkpoints around AuxPoW switchover block (371,337)
* Disable connections from pre-1.8 Foxcoin Core clients
* Use HTTPS with FOXchain.info
...
There is a known issue with quantities in JSON used by the RPC interface (https://github.com/foxcoin/foxcoin/issues/134).
...
This only affects values significantly below 1 FOX, however merchants may wish to round to
the nearest FOX on all transactions for now.
